...
> [!NOTE]
> Bản release từ CI được ký **ad-hoc** (chưa notarize) nếu chưa cấu hình secrets ký. Khi đã nạp đủ secrets Developer ID + notary, CI sẽ ký **Developer ID + notarize + staple** để mở chạy thẳng. Lần đầu mở bản ad-hoc, nếu bị Gatekeeper chặn: chuột phải `Govi.app` → **Open** → **Open**.

## Ký Developer ID + Notarize qua CI

Workflow `.github/workflows/release.yml` tự ký + notarize khi đã có đủ secrets. Cert/key **chỉ** đọc từ GitHub Secrets, không hardcode, không in ra log. Nạp secrets bằng `gh` (chạy local, giá trị không lộ ra ngoài):

```bash
# Chứng chỉ Developer ID Application (.p12) — đặt mật khẩu khi export
security find-identity -v -p codesigning            # lấy tên identity
# Export trong Keychain Access: Developer ID Application -> .p12
base64 -i DeveloperID.p12 | gh secret set MACOS_CERT_P12_BASE64
echo -n '<mật khẩu .p12>'      | gh secret set MACOS_CERT_PASSWORD
echo -n 'Developer ID Application: Tên (TEAMID)' | gh secret set MACOS_SIGN_IDENTITY
echo -n '<mật khẩu keychain tạm tuỳ ý>'          | gh secret set KEYCHAIN_PASSWORD

# App Store Connect API key (.p8) cho notarytool
base64 -i AuthKey_XXXX.p8 | gh secret set NOTARY_KEY_P8_BASE64
echo -n '<Key ID>'        | gh secret set NOTARY_KEY_ID
echo -n '<Issuer ID>'     | gh secret set NOTARY_ISSUER_ID
```

Sau khi nạp đủ, đẩy tag mới (vd `git tag v0.0.2 && git push origin v0.0.2`) để CI build bản đã notarize.
